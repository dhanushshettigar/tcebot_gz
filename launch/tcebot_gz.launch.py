...
from tcebot_gz.launch_tools.substitutions import TextJoin
...
def generate_launch_description():
    ...
    robots_arg = DeclareLaunchArgument(
        'robots', default_value="tcebot={x: 0., y: 0., z: 0.05, yaw: 0.};",
    ...
    params_file_arg = DeclareLaunchArgument(
        'params_file',
        default_value=PathJoinSubstitution([pkg_tcebot_gz, 'config', 'nav2_params.yaml']),
    ...
    gui_config_path = PathJoinSubstitution([pkg_tcebot_gz, 'config_gui', gui_config])
    nav2_flag = LaunchConfiguration('nav2')
    params_file = LaunchConfiguration('params_file')

    # Obtains world path.
    world_path = PathJoinSubstitution([pkg_tcebot_gz, 'worlds', world_name])
    log_world_path = LogInfo(msg=TextJoin(substitutions=["World path: ", world_path]))
    # Obtains the map path.
    map_path = PathJoinSubstitution([pkg_tcebot_gz, 'maps', map_name, TextJoin(substitutions=[map_name ,'.yaml'])])
    ...
    for robot_name in robots_list:
        ...
        robots_group = GroupAction(
            ...
            actions=[
                ...
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
                        os.path.join(pkg_tcebot_gz, 'launch', 'include', 'spawn_robot.launch.py')
                    ),
                    launch_arguments={
                        'entity': robot_name,
                ...
                Node(
                    ...
                    package='rviz2',
                    executable='rviz2',
                    arguments=['-d', os.path.join(pkg_tcebot_gz, 'rviz', 'tcebot_gz_nav2.rviz')],
                ...
                Node(
                    ...
                    package='rviz2',
                    executable='rviz2',
                    arguments=['-d', os.path.join(pkg_tcebot_gz, 'rviz', 'tcebot_gz.rviz')],
                    parameters=[{'use_sim_time': True}],
                    remappings=[
                        ('/tf', 'tf'),
                        ('/tf_static', 'tf_static'),
                ...
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
                        os.path.join(pkg_tcebot_gz, 'launch', 'include', 'gz_ros_bridge.launch.py')
                    ),
                    launch_arguments={
                        'entity': robot_name,
                    }.items(),
