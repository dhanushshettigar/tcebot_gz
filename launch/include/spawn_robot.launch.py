...
def get_robot_description() -> str:
    robot_description_file_path = os.path.join(PKG_tcebot_GZ, 'urdf', 'tcebot_gz.urdf.xacro')
    ...
    robot_desc = robot_desc.replace(
        'package://tcebot_description/', f'file://{PKG_tcebot_DESCRIPTION}/'
    )
    return robot_desc
...
def generate_launch_description():
    ...
    entity_argument = DeclareLaunchArgument(
        'entity', default_value='tcebot', description='Name of the robot.'
    )
    x_argument = DeclareLaunchArgument(
        'initial_pose_x',
        default_value='0.0',
        description='Initial x pose of tcebot in the simulation.',
    )
    y_argument = DeclareLaunchArgument(
        'initial_pose_y',
        default_value='0.0',
        description='Initial y pose of tcebot in the simulation',
    )
    z_argument = DeclareLaunchArgument(
        'initial_pose_z',
        default_value='0.1',
        description='Initial z pose of tcebot in the simulation.',
    )
    yaw_argument = DeclareLaunchArgument(
        'initial_pose_yaw',
        default_value='0.0',
        description='Initial yaw pose of tcebot in the simulation.',
    )
